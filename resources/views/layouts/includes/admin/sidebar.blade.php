@php
    ...
    $links = [
        ...
        [
            'name' => 'Doctores',
            'icon' => 'fa-solid fa-user-md', // Ícono de médico
    ...
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white">
        <ul class="space-y-2 font-medium">
            @foreach($links as $link)
                <li>
                    {{--Revisa si exites definido una llave llamada 'header'--}}
                    @isset($link['header'])
                    ...
                    @else
                        ...
                        @else
                            <a href="{{$link['href']}}" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 group {{ $link['active'] ? 'bg-gray-100 border border-gray-200' : '' }}">
                                <span class = "w-6 h-6 inline-flex justify-center items-center text-gray-500">
                                    <i class="{{$link['icon']}}"></i>
                                </span>
